...
import { PiSpinner, PiRobot, PiMagicWand, PiPencilSimple, PiCheck, PiWarning } from 'react-icons/pi';
...
export default function GetInspiration() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLastGenerated(prompt, data.imageUrl, null); // Clear any previous pattern
    } catch (err) {
  ...
  const handleCreatePattern = async () => {
    ...
    try {
      ...
      setLastGenerated(prompt, generatedImage, pattern); // Save pattern to store
    } catch (err) {
